...
namespace fapp_planner
{
    ...
    for (int i = 0; i < N; ++i)
    {
      ...
      for (int j = 0; j <= K; ++j)
      {
        ...
        for (auto state : objects_->states)
        {
            ...
            // 3D 状态获取
            Eigen::Vector3d obj_pos_3d(state.position.x, state.position.y, state.position.z);
            Eigen::Vector3d obj_vel_3d(state.velocity.x, state.velocity.y, state.velocity.z);
            
            // 预测障碍物在当前时刻的位置
            obj_pos_3d += (curT + t_gap_) * obj_vel_3d;
            
            // === 微创新：动态膨胀半径 ===
            // 物体速度越快，半径越大
            double vel_norm = obj_vel_3d.head(2).norm(); // 建议只用水平速度，或者用 .norm() 也可以
            double dynamic_radius = r_static_ + k_vel_inflation_ * vel_norm;
            double dynamic_radius_sqr = dynamic_radius * dynamic_radius; // r^2
            
            // === 距离计算 (2D 平面) ===
            // dist = r^2 - d^2
            // d^2 = (x-x_obj)^2 + (y-y_obj)^2
            Eigen::Vector2d diff_xy = (pos - obj_pos_3d).head(2); // 只取 XY 差异
            double dist_sq_xy = diff_xy.squaredNorm();
            double dist_constraint = dynamic_radius_sqr - dist_sq_xy;
            ...
            if (dist_constraint > 0)
            {
                ...
                // 1. Cost 计算: f(x) = w * (r^2 - d^2)^3
                costp += penalty_weight * dist_constraint * dist_constraint * dist_constraint;
                
                // 2. 位置梯度 (gradp): df/dp
                // df/dp = 3 * w * (r^2 - d^2)^2 * (-2 * (p - p_obj))
                // 注意：这里只能影响 X 和 Y，不能影响 Z
                Eigen::Vector2d grad_xy = penalty_weight * 3 * dist_constraint * dist_constraint * (-2) * diff_xy;
                gradp.head(2) += grad_xy; 
                // gradp.z() 保持为 0，不产生垂直推力
                
                // 3. 时间梯度 (gradt): df/dT
                // 这一项利用链式法则：df/dT = (df/dp) * (dp/dT) + (df/dp_obj) * (dp_obj/dT)
                // dp/dT = vel (无人机速度)
                // dp_obj/dT = obj_vel (障碍物速度)
                // 所以 df/dT = grad_xy .dot(vel_xy - obj_vel_xy)
                Eigen::Vector2d vel_rel_xy = (vel - obj_vel_3d).head(2);
                gradt += grad_xy.dot(vel_rel_xy);
                
                // 4. 前一段轨迹时间的梯度 (grad_prev_t)
                // 类似于 gradt，但只涉及障碍物位置的变化
                grad_prev_t += grad_xy.dot(-obj_vel_3d.head(2));
            }
        }
  ...
  void PolyTrajOptimizer::setParam(ros::NodeHandle &nh)
  {
    ...
    // 新增参数
    nh.param("optimization/k_vel_inflation", k_vel_inflation_, 0.05);
    nh.param("optimization/r_static", r_static_, 1.0);
  }
